...
def find_surfacing_zones(model):
    """
    Scans a model and returns a set of Z heights where pure horizontal surfaces
    exist, and those surfaces need CNC surfacing.

    Args:
        model (Mesh object): The STL model object to analyze.

    Returns:
        list[float]: A sorted list of Z coordinates where surfacing has to be
        done. If no surfacing zones are found, the list contains a single
        element [0].
...
def generate_surfacing_polygon(model_center_base,
    ...
    """
    Generate polygons for CNC surfacing operations based on detected heights of
    horizontal planes. This function is necessary because the surfacing
    operation must not machine nearby surfaces or elements of our part. The way
    that the polygon will be generated will vary if the surface is at the top
    of the part (there will be no nearby elements) or in the middle of the
    part. The function also "cleans" the surfaces by ignoring the faces that
    are too small to be machined.
    ...
    Returns:
        ...
        shapely MultiPolygons representing the faces to be surfaced at
        specified heights.
    """
    ...
    for i, height in enumerate(z_surfaces):
        ...
        if height == round(model.bounds[1][2], 2):
            ...
            section_surfaces = model.section_multiplane(
                plane_origin=model_center_base,
                plane_normal=[0, 0, 1],
                heights=[(height - 0.0075)])
            temporary_poly = MultiPolygon(
                list(section_surfaces[0].polygons_full))
            # If the surface is a MultiPolygon, there might be some elements
            # too small to be machined and we have to eliminate them
            if isinstance(temporary_poly, ShapelyMultiPolygon):
                cleaned_polys = [poly for poly in temporary_poly.geoms
                                 if poly.area > 0.5]
        ...
        # All other cases
            ...
            section_surfaces = model.section_multiplane(
                plane_origin=model_center_base, plane_normal=[0, 0, 1],
                heights=[(height - 0.0075), height, (height + 0.0075)])
                
            lower_offset_polygon = MultiPolygon(
                list(section_surfaces[0].polygons_full))
            upper_offset_polygon = MultiPolygon(
                list(section_surfaces[2].polygons_full))
            dirty_poly = lower_offset_polygon.difference(upper_offset_polygon)
            if isinstance(dirty_poly, ShapelyMultiPolygon):
                cleaned_polys = [poly for poly in dirty_poly.geoms
                                 if poly.area > 0.5]
...
def generate_raw_surfacing_toolpath(cleaned_z_surfaces,
                                    surfacing_tool_radius,
                                    surfacing_stepover,
                                    show_surfacing_toolpaths):
    """
    Generates surfacing toolpaths doing consecutive offsets of the polygons
    corresponding to surfaces that have to be surfaced.
    ...
    Note:
        This function does not connect the toolpaths generated for the same
        polygon. The tool will have to make "Z hops" between every offset.
    ...
    for i, poly in enumerate(cleaned_z_surfaces):
        ...
        # Generates toolpaths merging all offsets together in one
        # multilinestring
        ...
        if temp_toolpaths_list and show_surfacing_toolpaths:
            gpd.GeoSeries(temp_toolpaths_list[::]).plot()
            plt.show()
...
def generate_surfacing_data(surfaceable_heights,
                            ...
                            raw_surfacing_toolpath):
    """
    Generates a list of dictionaries that contain all the data needed to then
...
def sort_toolpaths(surfacing_data, surfacing_direction):
    """
    Sort the toolpaths in the given surfacing data based on their length
    (polygon area).

    This function iterates over the surfacing data and for each element that
    contains a MultiLineString under the key 'Surfacing_passes', it sorts the
    geometries based on the area of the corresponding polygon they form. It
    then replaces the original MultiLineString with a sorted version in the
    surfacing data.
    ...
    Note:
        This function assumes that the 'Surfacing_passes' key in the surfacing
        data will always contain a valid geometry or MultiLineString, otherwise
        it might throw an error. Ensure that the input data adheres to this
        format.
    """

    for element in surfacing_data:
        if isinstance(element['Surfacing_passes'],
                      shapely.geometry.MultiLineString):
            # Determining the sorting order based on chosen surfacing direction
            reverse_sort = True if surfacing_direction != 'climbing' else False

            # Sorting the toolpaths by their length (corresponding polygon
            # area)
            sorted_geoms = sorted(
                element['Surfacing_passes'].geoms,
                key=lambda geom: shapely.geometry.Polygon(geom).area,
                reverse=reverse_sort)

            # Replace the MultiLineString with a sorted multilinestring
            element['Surfacing_passes'] = (
                shapely.geometry.MultiLineString(sorted_geoms))
    return surfacing_data
