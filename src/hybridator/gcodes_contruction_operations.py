...
def is_clockwise(line_string):
    ...
    This function computes the signed area for the provided line string. A negative
    value indicates a clockwise order, while a positive value indicates a counterclockwise order.
    
    Args:
        line_string (shapely.geometry.linestring.LineString): The line string to check.
...
def reverse_line_string(line_string):
    ...
    This function takes a LineString and returns a new LineString with its coordinates
    in the reverse order.
    
    Args:
        line_string (shapely.geometry.linestring.LineString): The line string to be reversed.
    ...
    Note:
        The resulting LineString will have the same geometric properties as the input, but with its coordinates reversed.
    """
    return shapely.LineString(list(line_string.coords)[::-1])


def construct_cnc_contouring_gcode(
        gcode_datas, buffered_contours_list, contouring_speed,
        bridge_speed, print_tool_number, contour_tool_number,jump_value, layer_height,
        z_contouring_adjustment, write_isolate_contouring_code,
        contouring_shadow_pass, filename, write_folder):
    """
    Constructs CNC contouring G-code for a reprap 3D printer based on provided parameters.
    This function processes each layer of the G-code and generates contouring toolpahts.
    If `write_isolate_contouring_code` is set to True, the function will write the 
    contouring Gcode alone according to the write_folder given in the command-line.

    Args:
        gcode_datas (list of dict): Data for each G-code layer.
        buffered_contours_list (list of dict): Buffered contours for processing.
        ...
        write_isolate_contouring_code (bool): Whether to write the G-code to a file.
    ...
    negative_jump = [
        f'G91 G1 Z-{contouring_corrected_jump_value + layer_height}', 
        'G90'
    ]
    dict_milling_coord = {}
    ...
    for j, line in enumerate(gcode_datas):
        ...
        for buffered_type in ['buffered_int', 'buffered_ext']:
            for key in buffered_contours_list[j][buffered_type].keys():
                ...
                contour_type = 'intern' if buffered_type == 'buffered_int' else 'extern'
                listecoord.append(f';Milling {contour_type} {key} / Layer={z_layer_number} / Z ={z_layer_height}')
                
                for i, point in enumerate(contour):
                    x = f'X{round(point[0], 2)}'
                    y = f'Y{round(point[1], 2)}'
                    z = f'Z{round(z_layer_height - z_contouring_adjustment - layer_height, 2)}'
                    if i == 0:
                        listecoord.append(f'G0 {x} {y} F{bridge_speed}')
                        listecoord.append(f'{negative_jump[0]} F{bridge_speed}')
                        listecoord.append(negative_jump[1])
                    elif i > 0:
                        listecoord.append(f'G1 {x} {y} {z} F{contouring_speed}')
        ...
        listecoord.append(f'G1 Z{round(z_layer_height + layer_height, 2)} F8000')
        dict_milling_coord[f'{z_layer_number}'] = listecoord

    if write_isolate_contouring_code:
        with open(f'{write_folder}/HBD_{filename}_contouring.gcode', 'w', encoding="utf-8") as f:
            for key, line in dict_milling_coord.items():
                for line in dict_milling_coord[f'{key}']:
                    f.write(line + '\n')

    return dict_milling_coord

def construct_cnc_surfacing_gcode(
        surfacing_data, surfacing_speed, bridge_speed, surfacing_tool_number,
    ...
    Args:
        ...
        write_isolate_surfacing_code (bool): Whether to write the isolated surfacing code.
        surfacing_swoope_speed (float): Speed for surfacing swoope.
        surfacing_shadow_pass (int): Number of shadow passes for surfacing.
        surfacing_roughing_pass_value (float): Value for surfacing roughing pass.
    ...
    Returns:
        dict_surfacing_coord (dict): Dictionary containing the coordinates for surfacing.

    Notes:
        This function generates G-code for CNC surfacing operations based on the input parameters. 
        If `write_isolate_surfacing_code` is True, the generated G-code alone will be written 
        to the write_folder folder.
    ...
    positive_jump = [f'G91 G0 Z{surfacing_corrected_jump_value}', 'G90']
    ...
    for j, operation in enumerate(surfacing_data):
        ...
        list_surfacing_path.append(f'G0 Z{z_layer_height + surfacing_corrected_jump_value} F{bridge_speed}')
        ...
        if surfacing_roughing_pass:
            list_surfacing_path.append(f'\n;ROUGH PASS Surfacing Z={z_layer_height}')
            ...
            for toolpath in geometries:
                ...
                for i, coord in enumerate(list_coord):
                    ...
                    if i == 0:
                        ...
                        list_surfacing_path += negative_jump + [f'F{surfacing_swoope_speed}']
                    else:
                        list_surfacing_path.append(f'G1 {x} {y} {z} F{surfacing_speed}')
                list_surfacing_path.append(f'G0 Z{z_layer_height + surfacing_corrected_jump_value} F{surfacing_swoope_speed}')
        ...
        if surfacing_shadow_pass == 1:
            list_surfacing_path.append(f'\n;SHADOW Surfacing Z={z_layer_height}')
            for toolpath in geometries:
                list_coord = list(toolpath.coords)
                for i, coord in enumerate(list_coord):
                    x = f'X{round(coord[0], 3)}'
        ...
        if surfacing_shadow_pass == 2:
            list_surfacing_path.append(f'\n;SHADOW Surfacing N° 1 Z={z_layer_height}')
            for toolpath in geometries:
                list_coord = list(toolpath.coords)
                for i, coord in enumerate(list_coord):
                    x = f'X{round(coord[0], 3)}'
            ...
            list_surfacing_path.append(f'\n;SHADOW Surfacing N° 2 Z={z_layer_height}')
            for toolpath in geometries:
                list_coord = list(toolpath.coords)
                for i, coord in enumerate(list_coord):
                    x = f'X{round(coord[0], 3)}'
        ...
        list_surfacing_path.append(f'G1 Z{round(z_layer_height + layer_height, 2)} F8000')
        dict_surfacing_coord[f'{z_layer_height}'] = list_surfacing_path
        
    if write_isolate_surfacing_code:
        with open(f'{write_folder}/HBD_{filename}_surfacing_CNC.gcode', 'w', encoding="utf-8") as f:
...
def merge_contour_and_surfacing_codes(dict_surfacing_coord,dict_milling_coord,write_cnt_surf_merged_code,filename,write_folder):
    ...
    This function combines the G-code operations from `dict_surfacing_coord` (surfacing)
    and `dict_milling_coord` (contouring). If if `write_cnt_surf_merged_code` is True, it will
    write a file of gcodes alone.

    Args:
        dict_surfacing_coord (dict): Dictionary with Z heights as keys and their corresponding surfacing G-code as values.
        dict_milling_coord (dict): Dictionary with layer information as keys and their corresponding contouring G-code as values.
        write_cnt_surf_merged_code (bool): If True, writes the merged G-code to a file.
        filename (str): Name of the output file if `write_cnt_surf_merged_code` is True.
        write_folder (str): Destination folder for the output file if `write_cnt_surf_merged_code` is True.

    Returns:
        dict: Dictionary with layer information as keys and the merged G-code for both contouring and surfacing as values.
    ...
    for key in dict_milling_coord.keys():
        ...
        for clé in surfacing_heights:
            if float(z)>=float(clé):
                #print(f"match surfacing at Z={z} for layer{key}")
                surf_contenu=dict_surfacing_coord[clé]
                del(surf_contenu[-4]) #remove T2 call, because we are going to contour after...
                del(cnt_contenu[2]) #remove T3 call, because surf op already called it
    ...
    if write_cnt_surf_merged_code:
        with open(f'{write_folder}/HBD_{filename}_merged_CNC.gcode', 'w', encoding="utf-8") as f:
            for key,line in merged_cnt_surfacing_dict.items():
...
def hbd_fdm_cnc_merge_codes(gcode,gcode_datas,merged_CNT_surfacing_dict,filename,write_folder):
    ...
    This function inserts the CNC G-code operations from `merged_CNT_surfacing_dict` 
    at the correct positions in the original `gcode` based on the layer information 
    provided in `gcode_datas`. The merged G-code is saved to a file in the `write_folder`.

    Args:
        gcode (list[str]): Initial G-code sequence.
        gcode_datas (list[dict]): Information about each layer, including the layer's 
        ending line (`layer_ending_line`) and its number (`layer_number`).
        merged_CNT_surfacing_dict (dict): Dictionary with layer numbers as keys and 
        corresponding CNC G-code operations as values.
    ...
    Notes:
        - The function uses deep copying to ensure the original `gcode` remains unchanged.
        - The resulting G-code contains sequences from both FDM printing and CNC operations in 
        ...
        for i,data in enumerate(gcode_datas):
            if index==gcode_datas[i]['layer_ending_line']:
                ...
                if i==len(gcode_datas)-1:
                    temp_merged_gcode.insert(index, merged_CNT_surfacing_dict[layer])
                else:
                    temp_merged_gcode.insert(index+5, merged_CNT_surfacing_dict[layer])
    merged_gcode=[]
    ...
    with open(f'{write_folder}/HBD_{filename}.gcode', 'w', encoding="utf-8") as f:
